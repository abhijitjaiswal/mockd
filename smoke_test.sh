...
curl -s "$BASE/api/v1/recruitment-settings/positions/departments" | j | head -20
...
curl -s -X POST "$BASE/api/v1/recruitment-settings/positions/departments" \
     -H "Content-Type: application/json" -d '{"title":"","description":123}' | j

hr "5. Bad and missing query params -> 422, same shape"
curl -s "$BASE/api/v1/user/list?status=NotAStatus" | j
curl -s "$BASE/api/v1/user/list?page=abc" | j
echo "  and a missing REQUIRED query param:"
curl -s "$BASE/api/v1/metadata/cities" | j

hr "6. Error paths: force any documented status (QA's main lever)"
OP="$BASE/api/v1/recruitment-settings/positions/mappings/3fa85f64-5717-4562-b3fc-2c963f66afa6"
...
echo "  GET /api/v1/role/read/{role_id} documents only 200 and 422."
echo "  This server runs with --allow-undocumented-status, so QA can still force one:"
curl -s -w "\n  -> %{http_code}\n" "$BASE/api/v1/role/read/abc" -H "X-Mock-Status: 500"
...
printf '  empty     -> '
curl -s "$BASE/api/v1/user/list" -H "X-Mock-Scenario: empty" \
  | $PY -c "import sys,json; d=json.load(sys.stdin); print(d['message'], '| items:', len(d['data']['items']))"
printf '  page_full -> '
curl -s "$BASE/api/v1/user/list" -H "X-Mock-Scenario: page_full" \
...
curl -s "$BASE/api/v1/recruitment-settings/positions/mappings" -H "X-Mock-Nulls: on" | j | head -12

hr "10. Slow response, for loader and timeout tests"
curl -s -o /dev/null -w "  X-Mock-Delay: 1200 took %{time_total}s\n" \
     "$BASE/api/v1/metadata/countries" -H "X-Mock-Delay: 1200"

hr "11. Stateful CRUD across this API's verb-in-path routes"
CREATED=$(curl -s -X POST "$BASE/api/v1/user/create" -H "Content-Type: application/json" -d '{
...
printf '  read   -> %s\n' "$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/v1/user/read/$UID_")"
printf '  update -> %s\n' "$(curl -s -o /dev/null -w '%{http_code}' -X PUT "$BASE/api/v1/user/update/$UID_" \
